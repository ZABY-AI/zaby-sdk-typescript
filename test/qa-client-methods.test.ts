...
describe("Zaby (server SDK) — auth headers", () => {
  it("sends x-zaby-api-key from string", async () => {
    const transport = new MockTransport([
      { method: "GET", path: "/api/v1/provisioning/agentic-os/agents/usage", json: {} },
    ]);
...
describe("Zaby — all client URL routing", () => {
  ...
  it("routes agents.create correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/agents", status: 201, json: { id: "a1" } },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.agents.create({ name: "test" });
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents");
  });

  it("routes agents.attachMcpTool correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/agents/agent_1/mcp-tools", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.agents.attachMcpTool("agent_1", {});
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents/agent_1/mcp-tools");
  });

  it("routes agents.deploy correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/agents/agent_1/deployments", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.agents.deploy("agent_1", {});
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents/agent_1/deployments");
  });
  ...
  it("routes mcp methods correctly", async () => {
    const transport = createTransport([
      { method: "GET", path: "/api/v1/provisioning/agentic-os/mcp/catalog", json: [] },
      { method: "POST", path: "/api/v1/provisioning/agentic-os/mcp/servers", json: { id: "s1" } },
      { method: "POST", path: "/api/v1/provisioning/agentic-os/mcp/installations", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
  ...
  it("routes knowledgeBases methods correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/knowledge-bases", json: { id: "kb1" } },
      { method: "POST", path: "/api/v1/provisioning/agentic-os/knowledge-bases/kb1/documents/text", json: {} },
      { method: "POST", path: "/api/v1/provisioning/agentic-os/knowledge-bases/kb1/retrieve", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    ...
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/knowledge-bases");
    expect(transport.requests[1]?.path).toBe("/api/v1/provisioning/agentic-os/knowledge-bases/kb1/documents/text");
    expect(transport.requests[2]?.path).toBe("/api/v1/provisioning/agentic-os/knowledge-bases/kb1/retrieve");
  });

  it("routes memory methods correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/agents/memory-retrievals", json: {} },
      { method: "GET", path: "/api/v1/provisioning/agentic-os/agents/memory-items", json: [] },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.memory.retrieve({ query: "q" });
    await zaby.memory.listItems({});
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents/memory-retrievals");
    expect(transport.requests[1]?.path).toBe("/api/v1/provisioning/agentic-os/agents/memory-items");
  });

  it("routes intelligence methods correctly", async () => {
    const transport = createTransport([
      { method: "GET", path: "/api/v1/provisioning/agentic-os/agents/intelligence/signals", json: [] },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.intelligence.listSignals({});
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents/intelligence/signals");
  });

  it("routes approvals methods correctly", async () => {
    const transport = createTransport([
      { method: "POST", path: "/api/v1/provisioning/agentic-os/agents/runs/run_1/approvals/appr_1/approve", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.approvals.approve("run_1", "appr_1");
    expect(transport.requests[0]?.path).toBe(
      "/api/v1/provisioning/agentic-os/agents/runs/run_1/approvals/appr_1/approve"
    );
  });

  it("routes usage methods correctly", async () => {
    const transport = createTransport([
      { method: "GET", path: "/api/v1/provisioning/agentic-os/agents/usage?agentId=a1", json: {} },
    ]);
    const zaby = new Zaby({ apiKey: "test", transport });
    await zaby.usage.getAgentUsage({ agentId: "a1" });
    expect(transport.requests[0]?.path).toBe("/api/v1/provisioning/agentic-os/agents/usage?agentId=a1");
  });
});
